...
export class CustomersService extends SimpleService<ICustomer> {
  ...
  async search(query: dtoCustomerQuery, status: string, scope: string): Promise<ICustomer[]> {
  ...
  async createAdmin(document: dtoCustomer): Promise<ICustomer> {
    document.scope = 'admin'

    if (document.profile) {
      document.profile = await this.personService.addScope(
        (document.profile as IPerson)._id,
        document.scope
      )
    } else {
  ...
  async generateReferralCode(document: dtoCustomer | ICustomer): Promise<dtoCustomer | ICustomer> {
    ...
    if (document.referralCode) {
      if (await this.model.findOne({ referralCode: document.fromReferralCode }).exec()) {
        document.points = (await this.unitService.getValue()).invitationPoints
      } else {
  ...
  async create(document: dtoCustomer): Promise<ICustomer> {
    let customer: any
    ...
    if (document.profile) {
      ...
      document = await this.generateReferralCode(document) as dtoCustomer
  ...
  async new(document: ICustomer): Promise<ICustomer> {
    document.profile = document.profile as IPerson
    ...
    document = await this.generateReferralCode(document)
  ...
  async guestNew(document: ICustomer): Promise<ICustomer> {
    document.profile = document.profile as IPerson
  ...
  async convertFromGuest(document: dtoCustomer): Promise<ICustomer> {
  ...
  protected async sendAdminNotification(profile: IPerson): Promise<IAdminNotification> {
    const notification = {
      type: 'Customer',
      title: 'New Customer',
      message:
    ...
    return this.adminNotificationsService.create(notification as IAdminNotification)
  }

  async change(document: dtoCustomer): Promise<any> {
